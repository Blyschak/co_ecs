...
TEST(has_map, empty) {
    co_ecs::detail::hash_map<std::string, std::string> ht;
}
...
TEST(hash_map, basic_with_alloc) {
    co_ecs::detail::hash_map<std::string, std::string> ht{ std::allocator<std::pair<std::string, std::string>>() };
    ht.insert({ "hello", "world" });
    EXPECT_EQ(ht["hello"], "world");
    ht.insert({ "hey", "there" });
    EXPECT_EQ(ht["hey"], "there");
...
TEST(hash_map, construct_from_initializer_list) {
    co_ecs::detail::hash_map<std::string, std::string> ht = { { "hello", "world" }, { "hey", "there" } };
    EXPECT_EQ(ht["hey"], "there");
    EXPECT_EQ(ht["hello"], "world");
}
...
TEST(hash_map, construct_from_iterators) {
    ...
    co_ecs::detail::hash_map<std::string, std::string> ht{ vec.begin(), vec.end() };
    EXPECT_EQ(ht["hey"], "there");
    EXPECT_EQ(ht["hello"], "world");
}

TEST(hash_map, copy_ctor) {
    co_ecs::detail::hash_map<std::string, std::vector<std::string>> ht = { { "hello", { "world", "!" } },
        { "hey", { "there", "!" } } };
...
TEST(hash_map, move_ctor_unique_ptr) {
    co_ecs::detail::hash_map<std::string, std::unique_ptr<std::string>> ht;
...
TEST(hash_map, find) {
    co_ecs::detail::hash_map<int, int> ht;
    ht.emplace(1, 1);
    ht.emplace(2, 4);
    ht.emplace(4, 16);
...
TEST(hash_map, insert_and_erase) {
    co_ecs::detail::hash_map<std::string, int> ht{
        { "1", 1 },
        { "2", 2 },
        { "3", 3 },
        { "4", 4 },
