...
TEST(sparse_map, insert_remove_clear) {
    auto map = cobalt::asl::sparse_map<unsigned int, std::tuple<int, bool>>{};
...
TEST(sparse_map, iteration) {
    cobalt::asl::sparse_map<unsigned int, unsigned int> map = { { 1, 1 }, { 2, 4 }, { 3, 9 }, { 4, 16 }, { 5, 25 } };
    map.emplace(6, 36);
    map.erase(6);
    EXPECT_EQ(
...
TEST(sparse_map, insert_non_copiable) {
    cobalt::asl::sparse_map<unsigned int, std::unique_ptr<int>> map{};
