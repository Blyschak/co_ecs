...
/// node_base and weak links, or edges, to nodes which are used for search
...
/// strong child relation pointer recursively we will find that this graph is
...
private:
    ...
    /// @brief Link this node with the passed node. This method establishes
    ...
    ///
    /// @param value Value
    /// @param node Node to establish relationship with
    ...
    /// @brief Visit nodes starting from current node recursively in
...
public:
    ...
    /// @brief Visit all nodes starting from the root recursively in
    /// depth-first order
    ///
